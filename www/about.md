...
3. The KF fit does not include any dynamics (a.k.a. control inputs) and assumes time-invariant, additive, independent measurement and process noises (for the trend and the sinusoids). However, if error bars are in use, the relative change of the error bars is used to scale the measurement noise, which becomes time variable. The average of the actual measurement noise level used for the entire series will correspond to the measurement noise value indicated in the left panel. Data gaps are also accounted for in the increase of the process noise between observations.  
