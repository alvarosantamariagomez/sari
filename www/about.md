...
header-includes:
   - \usepackage{color}
   - \usepackage{courier}
title: 'SARI documentation - version mayo 2023'
author:
...
Current SARI version: *mayo 2023*  --  "We're not computers, Sebastian. We're physical"
...
Also, if there is a secondary series being used with the `correct` or `average` options, the reduced sampling will be obtained before the secondary series is used. This means that the primary and secondary series are being compared after their original sampling has been reduced.  
...
5. The `compute measurement noise` option will provide the most likely measurement noise level, within some bounds provided by the user, and with respect to the process noise also provided by the user. This is done through a maximum likelihood optimization of a preliminary UKF fit with respect to the provided process noise variances on the trend and sinusoids only. The optimization uses the Brent (1971) method, and it may take up to ~20 iterations of the UKF fit. The 95% CI of the estimated measurement noise standard deviation will be shown next to the input bounds on the left panel.  
6. The process noise values for the trend and sinusoidal variations are zero by defaul. Any positive value will make these parameters to change with time. When fitting a time-variable trend, a plot below the residual series will show the time series of the estimated trend, which is useful to assess whether the filter is too smooth or not, especially if other parameters were fitted together with the rate.  
...
2. The white noise and the random walk covariance matrices are obtained as described in Williams (2003), i.e. identity for white noise and linearly increasing variance from the first observation for random walk noise. For flicker noise, the covariance matrix is obtained using the fast Toeplitz solver as described in Bos et al. (2013).  
3. The power-law is a generalization of the "colored" noise processes, and thus it is not possible to estimate power-law with flicker or random walk, but it is compatible with white noise, though.  
...
1) Upload the series file from your computer using `browse file`, or from a web server via the URL, or from a Unix-like terminal using the [sari.sh script](https://github.com/alvarosantamariagomez/sari/blob/main/scripts/sari.sh), and set the series format and time units if necessary.  
...
21) Load the parameters of a `plate model` to check if your site is located where it should be and it is not moving away from its plate.  
