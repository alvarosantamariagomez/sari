...
header-includes:
   - \usepackage{color}
   - \usepackage{courier}
title: 'SARI documentation - version junio 2023'
author:
...
Current SARI version: *junio 2023*  --  "I’m sorry, Dave. I’m afraid I can’t do that"
...
If the series were computed with a constant integration period, like the common 24 h in GNSS position series, this option is useful to transform daily GNSS series with irregular sampling into daily GNSS series with regular sampling, i.e., one point every 24 h except when data gaps exist.  
...
6. The `compute measurement noise` option will provide the most likely measurement noise level, within some bounds provided by the user, and with respect to the process noise also provided by the user. This is done through a maximum likelihood optimization of a preliminary UKF fit with respect to the provided process noise variances on the trend and sinusoids only. The optimization uses the Brent (1971) method, and it may take up to ~20 iterations of the UKF fit. The 95% CI of the estimated measurement noise standard deviation will be shown next to the input bounds on the left panel.  
...
18) Use the estimated noise parameters to check the significance of the estimated offsets using the `offset verification` option and remove offsets from the model accordingly.  
